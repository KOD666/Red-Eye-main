...
INPUT_FILE = "detected.json"
SUSPICIOUS_INPUT_FILE = "suspicious.json"
...
def is_red_detected_app(app):
    # Parse vt_detection_rate
    vt_malicious = 0
    ...
    # Red: is_malware or score >= 61 or risk_level == 'red'
    return is_malware or score >= 61 or app.risk_level == "red"


def is_yellow_suspicious_app(app):
    if is_red_detected_app(app):
        return False
    score = app.threat_score or 0
    return score >= 30 or app.risk_level == "yellow"


def sync_detected_apps_file():
    """
    Reads the database for all current detected (red) apps.
    Saves/updates the detected.json file to only contain current detected apps.
    """
    if not SessionLocal:
    ...
    try:
        ...
        seen_hashes = set()
        unique_detected = []
        for app in current_detected:
            if app["sha256"] not in seen_hashes:
                seen_hashes.add(app["sha256"])
                unique_detected.append(app)
                
        with open(INPUT_FILE, "w") as f:
            json.dump(unique_detected, f, indent=4)
            
        print(f"[{time.strftime('%X')}] Synced {INPUT_FILE} - {len(unique_detected)} detected apps currently listed.")
        return unique_detected
    except Exception as e:
        print(f"[!] Error syncing detected apps: {e}")
        return []
    finally:
        db.close()
...
def sync_suspicious_apps_file():
    ...
    try:
        ...
        with open(SUSPICIOUS_INPUT_FILE, "w") as f:
            json.dump(unique_suspicious, f, indent=4)
            
        print(f"[{time.strftime('%X')}] Synced {SUSPICIOUS_INPUT_FILE} - {len(unique_suspicious)} suspicious apps currently listed.")
...
def scan_batch(apps):
    ...
    whitelist = set()
    _whitelist_file = "malware_free.json"
    if os.path.exists(_whitelist_file):
        try:
            with open(_whitelist_file, "r") as wf:
    ...
    for app in apps:
        ...
        if norm_sha in whitelist:
            print(f"[+] Hash {sha256[:12]}... found in local whitelist (malware_free.json). Skipping API call.")
            results.append({
                "app_name": app["app_name"],
                "package": app["package"],
                "version": app["version"],
    ...
    if apps_to_scan:
        print(f"[*] Starting concurrent check of {len(apps_to_scan)} uncached app hashes...")
        MAX_WORKERS = min(4, len(apps_to_scan))
...
def main():
    ...
    print("[*] Starting continuous scan_detected daemon (updating every 2 minutes)...")
    
    try:
        while True:
            # 1. Sync current detected apps list from DB (keeps detected.json updated)
            apps_to_scan = sync_detected_apps_file()
            ...
            if unscanned_apps:
                print(f"[*] Found {len(unscanned_apps)} new/unscanned detected apps. Scanning...")
                scan_batch(unscanned_apps)
                sync_detected_apps_file()
            else:
                print(f"[*] No new detected apps to scan.")
